...
# override in case colorama is not imported
class Fore:
...
%sArguments:%s
    ...
    if len(argv):
        if argv[0] == '-h':
            ...
            if not exists(argv[0]) and isfile(argv[0]):
                raise FileNotFoundError(Fore.RED+'file '+argv[0]+' does not exist.')
            ...
            for i in range(1, len(argv)): # set temporary options
                if i&1: # get option to set
                    ...
                    if current == 0:
                        minecraft_folder = argv[i]
                    elif current == 1:
                        show_warnings = int(argv[i])
                    elif current == 2:
                        show_code = int(argv[i])
                    elif current == 3:
                        colors = int(argv[i])
                        initcolors()
                    current = -1
        ...
        code = 6
    if code:
        raise SyntaxError('The syntax of the command is incorrect. Error code: %d' %code)
